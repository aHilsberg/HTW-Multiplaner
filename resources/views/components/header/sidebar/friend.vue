...
import {friendshipStatusText, friendshipStatusColor} from '@/scripts/helpers/basic'
import {acceptFriendshipCall, removeFriendshipCall} from '@/scripts/helpers/backendInteraction'
...
const removeFriend = () => removeFriendshipCall({data: {friend_id: props.friend.id}})
const acceptFriend = () => acceptFriendshipCall({data: {friend_id: props.friend.id}})
</script>

<template>
    <SidebarItem class="flex items-center gap-2"
                 :class="{ '!bg-green-50' : friend.friendshipState === FriendshipStatus.Befriended }">
        <div class="w-10 h-10 rounded-full bg-gray-200"/>
        <span class="text-lg">{{ friend.name }}</span>

        <div class="mx-auto" />

        <span :class="friendshipStatusColor(friend.friendshipState)" class="ml-3">
                {{
